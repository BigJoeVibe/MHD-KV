...
| **`KNOWN_LINE_CLASSES` nikdy nematchne** (nález executora + ověřeno managerem 14. 8.) = **krok B** | Set stringů × `line` je number → barevné odznaky se nezobrazí **nikde** (Moje trasy, Tabule, Hledat). Pre-existující, ne z J7-P2. **Joe rozhodl 14. 8.: nejen opravit typ, ale nahradit starou F1 paletu (5 linek) skutečnými barvami DPKV (21 linek)** + barva číslice se počítá podle kontrastu. Paleta, zdroj a čísla: `docs/DPKV_BARVY.md`. | ⭐ **ZADÁNO 14. 8.**, spec v `handoff.md` |
